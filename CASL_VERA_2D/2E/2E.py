...
# casl_pin_power is a plain local module, not a pyopensn/mpi4py binding -- unlike the imports in
# the guarded block above, it is never pre-bound as a global by console mode, so it must be
# imported unconditionally here rather than only under "opensn_console" not in globals().
sys.path.append(str(Path(__file__).resolve().parent.parent))
from casl_pin_power import compute_pin_powers  # noqa: E402 -- needs sys.path set up first

# MPIAllReduce is the console's own bound MPI wrapper (see py_app.cc/console.cc) -- mpi4py
# itself must not be imported under console mode, since the console already owns MPI
# initialization and exposes rank/size/MPIAllReduce directly. None signals
# casl_pin_power._gather_points to fall back to a real mpi4py allgather instead, only taken
# in the non-console branch above where mpi4py is the sole MPI mechanism available.
